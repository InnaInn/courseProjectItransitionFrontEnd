...
function PositionsTablePage() {
  ...
  const handleEdit = () => {
    if (selectedIds.length === 1) {
      ...
      alert(t('selectExactlyOne') || 'Please select exactly one position to edit.');
    }
  };
  ...
  const handleDuplicate = async () => {
    if (selectedIds.length === 1) {
      ...
      alert(t('selectExactlyOne') || 'Please select exactly one position to duplicate.');
    }
  };
  ...
  const renderTableBody = () => {
    if (loading) {
      return (
        <tr>
          <td colSpan={isHomePage ? 3 : (isCandidate ? 3 : 4)} className="px-3 sm:px-6 py-4 text-center text-gray-500 dark:text-gray-400 text-sm">
            {t('loading')}
          </td>
        </tr>
      );
    ...
    if (error) {
      return (
        <tr>
          <td colSpan={isHomePage ? 3 : (isCandidate ? 3 : 4)} className="px-3 sm:px-6 py-4 text-center text-red-500 dark:text-red-400 text-sm">
            {t('error')}: {error}
          </td>
        </tr>
      );
